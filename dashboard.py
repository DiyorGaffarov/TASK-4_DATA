...
st.markdown("""
    <h1 style="text-align:center; color:#2c3e50">📊 Sales Dashboard</h1>""", unsafe_allow_html=True)
st.divider()

data1_tab, data2_tab, data3_tab = st.tabs(["DATA1", "DATA2", "DATA3"])

orders_df = pd.read_csv('DATA1/data1/orders_clean.csv')
users_df = pd.read_csv('DATA1/data1/users_clean.csv')
books_df = pd.read_csv('DATA1/data1/books_clean.csv')
...
data1_top5_revenue_fig = go.Figure(data=[go.Table(
    ...
    cells=dict(
        values=[top5['rank'], top5['date'], top5['revenue']],
...
data1_top5_revenue_fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=220)
...
with data1_tab:
    ...
    st.plotly_chart(data1_top5_revenue_fig, use_container_width=True)
    ...
    with col_y:
        selected_year = st.selectbox('Year', ['All'] + years, key='year1')
    with col_m:
        selected_month = st.selectbox('Month', ['All'] + list(months.values()), key='month1')
    ...
    fig_line = px.line(filtered, x='date', y='revenue', title='Daily Revenue')
